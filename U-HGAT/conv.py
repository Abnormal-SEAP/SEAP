...
#             res_attn=None
#             if 'a' in graph.edata:
#                 res_attn = graph.edata.pop('a').detach()
#                 print("res_attn", res_attn.shape)
            # compute softmax
...
#             print("rst", rst.shape)
            if self.res_fc is not None:
                resval = self.res_fc(h_dst[:graph.number_of_dst_nodes()]).view(h_dst[:graph.number_of_dst_nodes()].shape[0], -1, self._out_feats)
                rst = rst + resval
            # bias
            if self.bias:
